...
AFRAME.registerComponent('palette', {
  schema: {
     ...
     colors: {type:'string',  // Mirar cambio
  ...
  init: function () {
    ...
    el.addEventListener(data.event, evt => {
      ...
      //
      //   for(var a=0;a<10;a++) // position.length
      //        {
      ...
      //          //log de obj
      //          obj_colors.add(obj); // Mirar los valores
      //          //log de obj_colors
      //        }
  ...
  GetColorObj:function(){
    ...
    el.addEventListener(data.event, evt => {
      const post_evt = evt.detail.intersection.point
      const obj = document.querySelector("#" + data.id).firstChild;
      const objjj = this.el.getObject3D('Colors');

      // console.log(objjj.children[1].position.x);
      // console.log(post_evt.x);
      // console.log(objjj.children[1].position.x + 0.2);
      if (obj != null){
        if ( post_evt.x < objjj.children[1].position.x + 0.2|| post_evt.x > objjj.children[1].position.x -0.2 ){
